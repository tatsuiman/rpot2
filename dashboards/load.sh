...
do
    ...
    ${CURL} -s -XPUT ${ELASTICSEARCH}/${KIBANA_INDEX}/search/${NAME} \
        -d @${TMP_SED_FILE}|jq '._shards.successful'
    sleep 0.1
done
rm ${TMP_SED_FILE}
...
do
    ...
    ${CURL} -s -XPUT ${ELASTICSEARCH}/${KIBANA_INDEX}/visualization/${NAME} \
        -d @${file} |jq '._shards.successful'
    sleep 0.1
done
...
do
    ...
    ${CURL} -s -XPUT ${ELASTICSEARCH}/${KIBANA_INDEX}/dashboard/${NAME} \
        -d @${file} |jq '._shards.successful'
    sleep 0.1
done
...
do
    ...
    ${CURL} -s -XPUT ${ELASTICSEARCH}/${KIBANA_INDEX}/index-pattern/${NAME} \
        -d @${file} |jq '._shards.successful'
    sleep 0.1
done
