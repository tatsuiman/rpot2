...
function main() {
	...
	if [ ! -e elasticsearch-5.5.2.deb ]; then
		wget https://artifacts.elastic.co/downloads/elasticsearch/elasticsearch-5.5.2.deb -O elasticsearch-5.5.2.deb
	fi
	for i in $(seq $node_num)
	do
		for j in $(seq $shard_num)
		do
			lxc file push elasticsearch-5.5.2.deb es-node${i}-shard${j}/root/
			...
			lxc exec es-node${i}-shard${j} -- sh -c "dpkg -i elasticsearch-5.5.2.deb"
